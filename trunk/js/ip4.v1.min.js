...
ip4.data.DataReader.prototype.read=function(callback){if(callback)this.callback(callback);var uri=this.uri(),data=this.data(),mimetype=this.mimetype(),adapter=this.adapter(),parse;if(adapter)adapter.clearCache();if(uri){if(ip4.util.isUrl(uri)){switch(mimetype){case"text/csv":parse=d3.csv;break;case"text/tsv":parse=d3.tsv;break;case"application/xml":parse=d3.xml;break;default:parse=d3.json;}
parse(uri,function(d){return adapter?adapter.adapt(d):null},function(error,data){callback.call(this,error,data)});}else{data=d3.select(uri).text();}}
if(data){switch(mimetype){case"text/csv":parse=d3.csv.parse;break;case"text/tsv":parse=d3.tsv.parse;break;case"application/xml":parse=d3.xml.parse;break;default:switch(typeof data){case"string":parse=JSON.parse;break;case"function":data=data();break;}}
