...
return points;}if(!ip4.color)ip4.color={}
...
if(group&&y){if(ip4.log.log())console.log(this.toString()+" Adapting group and y...");data.forEach(function(d,i){d.name=group[1]?group[1](d[group[0]].trim()):d[group[0]].trim();delete d[group[0]];d.values=y[0].map(function(sub,i){var value={"x":sub,"y":y[1]?y[1](d[sub]):+d[sub]};delete d[sub];return value;});});}
if(group&&x){if(ip4.log.log())console.log(this.toString()+" Adapting group and x...");data=group[0].map(function(serie){return{"name":serie,"values":data.map(function(d){return{"x":x[1]?x[1](d[x[0]].trim()):d[x[0]].trim(),"y":group[1]?group[1](d[serie]):+d[serie]}})}});}
...
if(!ip4.template)ip4.template={};ip4.template.PieChart=function(){if(ip4.log.log())console.log(this.toString()+" Constructor called");ip4.document.Document.call(this);this.svgDecorator=new ip4.decorator.SvgDecorator();this.tooltipDecorator=new ip4.decorator.TooltipDecorator();}
ip4.template.PieChart.prototype=Object.create(ip4.document.Document.prototype,{constructor:{configurable:true,enumerable:true,value:ip4.template.PieChart,writable:true}});ip4.template.PieChart.prototype.toString=function(){return"[ip4 PieChart]";}
ip4.template.PieChart.prototype.defaultConfiguration=function(){this.parent.classed("pieChart",true);var min=200,max=1000;return{"data":{"value":function(){return[{"x":"Group A","y":ip4.util.getRandomMinMax(min,max)},{"x":"Group B","y":ip4.util.getRandomMinMax(min,max)},{"x":"Group C","y":ip4.util.getRandomMinMax(min,max)},{"x":"Group D","y":ip4.util.getRandomMinMax(min,max)},{"x":"Group E","y":ip4.util.getRandomMinMax(min,max)}]}},"svg":{"size":{"margin":{"top":20,"right":10,"bottom":40,"left":10}},"metadata":{"dc:format":"application/ip4"}},"d3":{"pie":d3.layout.pie().sort(null),"arc":d3.svg.arc(),"innerRadius":0.33,"color":d3.scale.ordinal().range(ip4.color.default_palette),"yFormat":d3.format(",.2f"),"transitions":{"path":d3.transition().duration(1500).ease("linear"),"label":d3.transition().duration(1500).ease("linear")}}}}
ip4.template.PieChart.prototype.draw=function(){if(ip4.log.log())console.log(this.toString()+" Drawing with parentElement:",this.parentElement(),", configuration:",this.configuration());if(ip4.log.time())console.time(this.toString()+" Draw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,updateInterval=con.data.updateInterval;if(!ip4.util.isSvgSupport()){if(ip4.log.warn())console.warn(this.toString()+" Browser has no SVG support");new ip4.decorator.NoSvgSupportDecorator().draw(parent,con);return;}
this.svgDecorator.draw(parent,con);var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom,radius=Math.min(w,h)/2;parent.select("g.main").attr("transform","translate("+w/2+","+h/2+")");con.d3.pie.value(function(d){return+d.y;});con.d3.arc.outerRadius(radius);con.d3.arc.innerRadius(con.d3.innerRadius*radius);this.tooltipDecorator.draw();if(updateInterval&&updateInterval>0.0){if(ip4.log.info())console.info(this.toString()+" Starting data update interval "+updateInterval+" ms...");var self=this;window.setInterval(function(){self.update();},+updateInterval);}
if(ip4.log.info())console.info(this.toString()+" Updating...");this.update();if(ip4.log.time())console.timeEnd(this.toString()+" Draw");return this;}
ip4.template.PieChart.prototype.redraw=function(data){if(ip4.log.log())console.log(this.toString()+" Redrawing...");if(ip4.log.time())console.time(this.toString()+" Redraw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,pie=con.d3.pie,arc=con.d3.arc,xFormat=con.d3.xFormat,yFormat=con.d3.yFormat,color=con.d3.color,transitions=con.d3.transitions,updateInterval=con.data.updateInterval;var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom;var g=parent.select("g.main"),tooltip=d3.selectAll(".tooltip",".ip4");var arcs=g.datum(data).selectAll("g");var path=arcs.select("path").data(pie).attr("class","arc update");var label=arcs.select("text").data(pie).attr("class","text update");transitions.path.each(function(){path.transition().attrTween("d",arcTween);});transitions.label.each(function(){label.transition().attr("transform",function(d){return"translate("+arc.centroid(d)+")";});});var gEnter=path.enter().append("g");gEnter.append("path").attr("class","arc enter").attr("fill",function(d){return color(d.data.x);}).attr("d",arc).each(function(d){this._current=d;}).on("mouseover",function(d){d3.select(this).classed("selected",true);tooltip.html((xFormat?xFormat()(d.data.x):d.data.x)
+"<br/>"
...
ip4.template.pieChart=function(){return new ip4.template.PieChart();}
ip4.pieChart=function(){return ip4.template.pieChart();}