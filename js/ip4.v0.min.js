...
ip4.color.name2hex=function(name){return(typeof name==="string")?(ip4.color.names[name.toLowerCase()]||name):name;}
...
ip4.color.complement=function(color,type){var a=ip4.color.name2hex(color),c=1,d=180,f=0;switch(type){case"split":c=3;f=d=180;break;case"double":c=5,f=d=180;}
return ip4.color.rotationalDispersion(a,c,"hue",f,d);}
ip4.color.triadic=function(color){var a=ip4.color.name2hex(color);return ip4.color.rotationalDispersion(a,3,"hue",360,0);}
ip4.color.tetradic=function(color){var a=ip4.color.name2hex(color);return ip4.color.rotationalDispersion(a,4,"hue",360,0);}
ip4.color.pentadic=function(color){var a=ip4.color.name2hex(color);return ip4.color.rotationalDispersion(a,5,"hue",360,0);}
ip4.color.rotationalDispersion=function(color,count,type,scope,rotation){var a=ip4.color.name2hex(color),b=count,c=type,d=scope,f=rotation;b="number"!==typeof b?3:b;c="string"!==typeof c?3:c;d="number"!==typeof d?360:d;f="number"!==typeof f?0:f;var e=ip4.color.hex2hsv(a);a=e[0];var j=e[1],e=e[2],h,g=[],k;h="hue"===c&&(360===d||0===d)?d/b:d/(b-1);d=360===d?0:ip4.color.degrees(ip4.color.degrees(a,f),-1*d/2);for(k=0;k<b;k+=1)
...
return g;}
ip4.color.degrees=function(color,b){var a=ip4.color.name2hex(color);a+=b;360<a?a-=360:0>a&&(a+=360);return a;}
ip4.color.shades=function(color,count){count=count||10;return ip4.color.rotationalDispersion(color,count+1,"saturation",100);}
...
ip4.document.Document.prototype.parentElement=function(parentElement){if(parentElement===undefined){if(!this.pel){if(ip4.log.warn())console.warn(this.toString()+" No parent element defined. Appending <div> to <body> element");this.pel=d3.select("body").append("div");}
return this.pel.classed("ip4",true);}
if(parentElement===null){this.pel=undefined;return this;}
this.pel=parentElement;if(typeof parentElement==="string"){var parentElementId=parentElement.indexOf("#")>-1?parentElement:("#"+parentElement);if(ip4.log.info())console.info(this.toString()+" Selecting parentElement with id: "+parentElementId);this.pel=d3.select(parentElementId);}
return this;}
...
ip4.data.TableAdapter=function(category,value){if(ip4.log.log())console.log(this.toString()+" TableAdapter constructor called");this.cat=category;this.val=value;}
...
ip4.data.TableAdapter.prototype.category=function(category){if(!category)return this.cat;if(ip4.log.log())console.log(this.toString()+" Setting category to: "+category);this.cat=category;return this;}
ip4.data.TableAdapter.prototype.value=function(value){if(!value)return this.val;if(ip4.log.log())console.log(this.toString()+" Setting value to: "+value);this.val=value;return this;}
ip4.data.TableAdapter.prototype.adapt=function(data){if(ip4.log.log())console.log(this.toString()+" Adapting data (categroy: "+this.category()+", value: "+this.value()+")");if(ip4.log.time())console.time(this.toString()+" Adapt");var cat=this.category(),val=this.value();if(typeof cat[0]==="string"&&typeof val[0]==="string"){if(ip4.log.log())console.log(this.toString()+" Using paradigm BarChart for adapt");data.forEach(function(d){d.group=cat[1]?cat[1](d[cat[0]]):d[cat[0]];delete d[cat[0]];d.value=val[1]?val[1](d[val[0]]):+d[val[0]];delete d[val[0]];});}
if(typeof cat[0]==="string"&&typeof val[0]==="object"){if(ip4.log.log())console.log(this.toString()+" Using paradigm GroupedBarChart for adapt");var subcats=val.map(function(d){return d[0].trim();});data.forEach(function(d,i){d.group=cat[1]?cat[1](d[cat[0]].trim()):d[cat[0]].trim();delete d[cat[0]];d.values=subcats.map(function(subcat,i){var value={"group":subcat,"value":val[i][1]?val[i][1](d[subcat]):+d[subcat]};delete d[subcat];return value;});});}
...
ip4.data.tableAdapter=function(category,value){return new ip4.data.TableAdapter(category,value);}
...
ip4.decorator.FigureDecorator=function(){if(ip4.log.log())console.log(this.toString()+" FigureDecorator constructor called");}
ip4.decorator.FigureDecorator.prototype={constructor:{configurable:true,enumerable:true,value:ip4.decorator.FigureDecorator,writable:true}}
ip4.decorator.FigureDecorator.prototype.toString=function(){return"[ip4 FigureDecorator]";}
ip4.decorator.FigureDecorator.prototype.draw=function(parentElement,configuration){if(ip4.log.log())console.log(this.toString()+" Drawing in parentElement:",parentElement,", configuration:",configuration);var css=configuration.style?configuration.style.css:undefined,figcaption=configuration.html?configuration.html.figcaption:undefined,size=configuration.svg?configuration.svg.size:undefined,metadata=configuration.svg?configuration.svg.metadata:undefined;if(css){if(ip4.log.log())console.log(this.toString()+" Adding CSS style.");parentElement.append("style").attr("type","text/css").text(css);}
if(figcaption){if(ip4.log.log())console.log(this.toString()+" Adding figure and SVG elements.");var figure=parentElement.append("figure"),svg=figure.append("svg");}else{if(ip4.log.log())console.log(this.toString()+" Adding SVG element.");var svg=parentElement.append("svg");}
this.g=svg.append("g").attr("class","main");if(size){svg.attr("width",size.width);svg.attr("height",size.height);if(size.margin){this.g.attr("transform","translate("+size.margin.left
+","+size.margin.top+")");}}
...
if(figcaption){figure.append("figcaption").text(figcaption);}}
...
if(!ip4.template)ip4.template={};ip4.template.BarChart=function(){if(ip4.log.log())console.log(this.toString()+" Constructor called");ip4.document.Document.call(this);this.figureDecorator=new ip4.decorator.FigureDecorator();this.axisDecorator=new ip4.decorator.AxisDecorator();this.tooltipDecorator=new ip4.decorator.TooltipDecorator();}
...
ip4.template.BarChart.prototype.defaultConfiguration=function(){var dWidth=this.parentElement().node().offsetWidth,dSize={"width":dWidth,"height":dWidth*0.6,"margin":{"top":20,"right":10,"bottom":40,"left":60}};var size=ip4.util.isPath(this.configuration,"svg.size")?this.configuration.svg.size:dSize;var w=size.width-(size.margin.right||0)-(size.margin.left||0),h=size.height-(size.margin.top||0)-(size.margin.bottom||0);return{"data":{"value":function(){return[{"group":"group A","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group B","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group C","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group D","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group E","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group F","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group G","value":ip4.util.getRandomMinMax(-1000,1000)},{"group":"group H","value":ip4.util.getRandomMinMax(-1000,1000)}]}},"style":{"css":".x.axis path { display: none; }"},"svg":{"size":dSize,"metadata":{"dc:format":"application/ip4"}},"d3":{"xScale":d3.scale.ordinal().rangeRoundBands([0,w],.15,.3),"yScale":d3.scale.linear().range([h,0]).nice(),"yLabel":"y-axis","xAxis":d3.svg.axis(),"yAxis":d3.svg.axis().orient("left").tickFormat(d3.format(",.3f")),"color":d3.scale.ordinal().range(ip4.color.random(20)),"transitions":{"groups":d3.transition().duration(1500).ease("linear"),"xAxis":d3.transition().duration(1500).ease("linear"),"yAxis":d3.transition().duration(1500).ease("linear"),"zeroline":d3.transition().duration(1500).ease("linear")}}}}
ip4.template.BarChart.prototype.draw=function(){if(ip4.log.log())console.log(this.toString()+" Drawing with parentElement:",this.parentElement(),", configuration:",this.configuration());if(ip4.log.time())console.time(this.toString()+" Draw");var pel=this.parentElement(),con=this.configuration(),x=con.d3.xScale,y=con.d3.yScale,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,yLabel=con.d3.yLabel,updateInterval=con.data.updateInterval;var w=con.svg.size.width-(con.svg.size.margin.right||0)-(con.svg.size.margin.left||0),h=con.svg.size.height-(con.svg.size.margin.top||0)-(con.svg.size.margin.bottom||0);if(!ip4.util.isSvgSupport()){if(ip4.log.warn())console.warn(this.toString()+" Browser has no SVG support");new ip4.decorator.NoSvgSupportDecorator().draw(pel,con);return;}
this.figureDecorator.draw(pel,con);this.axisDecorator.draw(pel.select("g.main"),xAxis,yAxis,x,y,w,h,null,yLabel);this.tooltipDecorator.draw();if(updateInterval&&updateInterval>0.0){if(ip4.log.info())console.info(this.toString()+" Starting data update interval "+updateInterval+" ms...");var self=this;window.setInterval(function(){self.update();},+updateInterval);}
...
ip4.template.BarChart.prototype.redraw=function(data){if(ip4.log.log())console.log(this.toString()+" Redrawing...");if(ip4.log.time())console.time(this.toString()+" Redraw");var pel=this.parentElement(),con=this.configuration(),x=con.d3.xScale,y=con.d3.yScale,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,color=con.d3.color,transitions=con.d3.transitions,updateInterval=con.data.updateInterval;var w=con.svg.size.width-(con.svg.size.margin.right||0)-(con.svg.size.margin.left||0),h=con.svg.size.height-(con.svg.size.margin.top||0)-(con.svg.size.margin.bottom||0);var g=pel.select("g.main"),tooltip=d3.selectAll(".tooltip",".ip4");x.domain(data.map(function(d){return d.group;}));y.domain([d3.min(data,function(d){return d.value>0?0:d.value;}),d3.max(data,function(d){return d.value;})]);var groups=g.selectAll(".group").data(data,function(d){return d.group;});groups.attr("class","group update").attr("width",x.rangeBand()).attr("x",function(d){return x(d.group);});groups.enter().append("rect").attr("class","group enter").attr("width",x.rangeBand()).attr("x",function(d){return x(d.group);}).style("fill",function(d){return color(d.group);}).on("mouseover",function(d){d3.select(this).classed("selected",true);tooltip.html((xAxis.tickFormat()?xAxis.tickFormat()(d.group):d.group)
+"<br/>"
+(yAxis.tickFormat()?yAxis.tickFormat()(d.value):d.value)).style("left",(d3.event.pageX+7)+"px").style("top",(d3.event.pageY-28)+"px").classed("selected",true);}).on("mousemove",function(d){tooltip.style("left",(d3.event.pageX+7)+"px").style("top",(d3.event.pageY-28)+"px");}).on("mouseout",function(d){d3.select(this).classed("selected",false);tooltip.html("").classed("selected",false);});groups.exit().attr("class","group exit").remove();transitions.groups.each(function(){groups.transition().attr("y",function(d){return d.value>0?y(d.value):y(0);}).attr("height",function(d){return d.value>0?h-y(d.value+y.domain()[0]):h-y(y.domain()[0]-d.value);})});this.axisDecorator.redraw(data,g,xAxis,yAxis,x,y,w,h,transitions);if(ip4.log.time())console.timeEnd(this.toString()+" Redraw");return this;}
