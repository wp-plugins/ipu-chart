...
var ip4=(ip4||{});if(!ip4.load)ip4.load={}
ip4.load.Loader=function(configuration,parentElement,dependencies){if(ip4.log.log())console.log(this.toString()+" Constructor called");this.configuration=configuration;this.parentElement=parentElement;this.dependencies=dependencies;}
...
ip4.load.Loader.prototype.loadJavascript=function(dependencies){for(var i in dependencies["application/javascript"]){var url=dependencies["application/javascript"][i];if(ip4.log.info())console.info(this.toString()+" Loading javascript "+url);var script=document.createElement("script")
script.setAttribute("src",url);script.setAttribute("type","application/javascript");script.setAttribute("charset","utf-8");document.head.appendChild(script);}}
ip4.load.Loader.prototype.loadCSS=function(dependencies){for(var i in dependencies["text/css"]){var url=dependencies["text/css"][i];if(ip4.log.info())console.info(this.toString()+" Loading css "+url);var link=document.createElement("link")
link.setAttribute("href",url);link.setAttribute("type","text/css");link.setAttribute("rel","stylesheet");link.setAttribute("charset","utf-8");document.head.appendChild(link);}}
ip4.load.Loader.prototype.run=function(){if(this.dependencies){if(ip4.log.log())console.log(this.toString()+" Loading dependencies:",this.dependencies);this.loadJavascript(this.dependencies);this.loadCSS(this.dependencies);}
if(ip4.log.log())console.log(this.toString()+" Drawing configurations");if(this.configuration.log){if(ip4.log.info())console.info(this.toString()+" Setting ip4.log.level.current="+ip4.log.toString(this.configuration.log));ip4.log.level.current=this.configuration.log;}
if(!this.configuration.template){if(ip4.log.error())console.error(this.toString()+" Template in configuration "+c+" not found: "+this.configuration.template);}else{if(!(this.parentElement||this.configuration.parentElement))
if(ip4.log.warn())console.warn(this.toString()+" No parent element defined. Appending <div> to <body> element");if(ip4.log.log())console.log(this.toString()+" Running template: "+this.configuration.template+", configuration:",this.configuration);this.configuration.template.parentElement(this.parentElement||this.configuration.parentElement||d3.select("body").append("div")).configuration(this.configuration).draw();}}
ip4.draw=function(configuration,parentElement,dependencies){new ip4.load.Loader(configuration,parentElement,dependencies).run();}
...
ip4.util.mixin=function(receiver,supplier){if(supplier==null||receiver==null)
return receiver;for(var p in supplier){if(p==="transitions"){receiver[p]=receiver[p]||{}
for(var t in supplier[p]){if(true){receiver[p][t]=supplier[p][t];}}}else{if(!receiver.hasOwnProperty(p)&&supplier.hasOwnProperty(p))
receiver[p]=supplier[p];if(typeof supplier[p]==="object"&&supplier.hasOwnProperty(p))
this.mixin(receiver[p],supplier[p]);}}
return receiver;}
...
ip4.document.Document.prototype.parentElement=function(parentElement){if(parentElement===undefined){if(!this.parent){if(ip4.log.warn())console.warn(this.toString()+" No parent element defined. Appending <div> to <body> element");this.parent=d3.select("body").append("div");}
return this.parent.classed("ip4",true);}
if(parentElement===null){this.parent=undefined;return this;}
this.parent=parentElement;if(typeof parentElement==="string"){var parentElementId=parentElement.indexOf("#")>-1?parentElement:("#"+parentElement);if(ip4.log.info())console.info(this.toString()+" Selecting parentElement with id: "+parentElementId);this.parent=d3.select(parentElementId);}
return this;}
ip4.document.Document.prototype.innerSize=function(){if(ip4.log.log())console.log(this.toString()+" Calculating innerSize...");var w=this.parentElement().node().offsetWidth,h=w*0.6,margin=this.configuration().svg.size.margin;w=w-(margin.right||0)-(margin.left||0);h=h-(margin.top||0)-(margin.bottom||0);return{"width":w,"height":h}}
ip4.document.Document.prototype.configuration=function(configuration){if(configuration===undefined){if(ip4.log.log())console.log(this.toString()+" Calculating configuration...");if(ip4.log.time())console.time(this.toString()+" Configuration");if(this.defaultConfiguration()){if(ip4.log.log())console.log(this.toString()+" Mixin default configuration");this.con=(this.con||{});ip4.util.mixin(this.con,this.defaultConfiguration());}
if(ip4.log.time())console.timeEnd(this.toString()+" Configuration");return this.con;}
if(configuration===null){if(ip4.log.log())console.log(this.toString()+" Resetting configuration");this.con={};ip4.util.mixin(this.con,this.defaultConfiguration());return this;}
if(typeof configuration==="string"){if(ip4.log.log())console.log(this.toString()+" Parsing configuration...");var self=this;return this.configuration(JSON.parse(configuration,function(key,value){if(ip4.log.log())console.log(self.toString()+" Parsing key: "+key+", value: ",value);switch(key){case"adapter":case"xScale":case"x0Scale":case"x1Scale":case"yScale":case"xAxis":case"yAxis":case"color":if(ip4.log.log())console.log(self.toString()+" Creating function for: "+key);return new Function("return "+value)();default:return value;}}));}
this.con=configuration;return this;}
ip4.document.Document.prototype.update=function(){if(ip4.log.log())console.log(this.toString()+" Updating...");if(ip4.log.time())console.time(this.toString()+" Update");var con=this.configuration(),value=con.data.value,uri=con.data.uri,mimetype=con.data.mimetype,adapter=con.data.adapter,reader=new ip4.data.Reader(),self=this;if(value){if(ip4.log.log())console.log(this.toString()+" Loading configuration's inline data");var data=reader.readLocal(value,mimetype);if(adapter){data=adapter.adapt(data);}
if(ip4.log.time())console.timeEnd(self.toString()+" Update");self.redraw(data);return;}else if(ip4.util.isUrl(uri)){if(ip4.log.log())console.log(this.toString()+" Loading data from url "+uri);reader.read(uri,mimetype).get().on("progress",function(){if(ip4.log.log())console.log(reader.toString()+" Progress...");}).on("load",function(data){if(ip4.log.log())console.log(reader.toString()+" Load:",data);if(adapter){data=adapter.adapt(data);}
if(ip4.log.time())console.timeEnd(reader.toString()+" Update");self.redraw(data);}).on("error",function(error){if(ip4.log.error())console.error(reader.toString()+" Read data error:",error);if(ip4.log.time())console.timeEnd(self.toString()+" Update");});}else{if(ip4.log.log())console.log(this.toString()+" Loading document's data "+uri);var data=reader.read(uri,mimetype);if(adapter){data=conf.data.adapter.adapt(data);}
if(ip4.log.time())console.timeEnd(self.toString()+" Update");self.redraw(data);}}
ip4.document.document=function(parentElemement,configuration){return ip4.document.Document(parentElemement,configuration);}
if(!ip4.data)ip4.data={}
ip4.data.Reader=function(){if(ip4.log.log())console.log(this.toString()+" Constructor called");}
ip4.data.Reader.prototype={constructor:{configurable:true,enumerable:true,value:ip4.data.Reader,writable:true}}
ip4.data.Reader.prototype.toString=function(){return"[ip4 Reader]";}
ip4.data.Reader.prototype.read=function(uri,mimetype,callback){if(ip4.log.log())console.log(this.toString()+" Reading uri: "+uri+" with mimetype: "+mimetype+", callback: "+callback);var mimetype=mimetype||"application/json";if(ip4.util.isUrl(uri)){switch(mimetype){case"application/json":return d3.json(uri,callback);case"text/csv":return d3.csv(uri,callback);case"text/tsv":return d3.tsv(uri,callback);case"application/xml":return d3.xml(uri,callback);case"text/html":return d3.html(uri,callback);case"text/plain":return d3.text(uri,callback);}}else{var text=d3.select(uri).text();switch(mimetype){case"application/json":return JSON.parse(text);case"text/csv":return d3.csv.parse(text);case"text/tsv":return d3.tsv.parse(text);case"application/xml":return d3.xml.parse(text);}}}
ip4.data.Reader.prototype.readLocal=function(value,mimetype){if(ip4.log.log())console.log(this.toString()+" Reading local data value:",value,", mimetype: "+mimetype+", typeof value: "+(typeof value));var mimetype=mimetype||"application/json";switch(mimetype){case"application/json":switch(typeof value){case"string":return JSON.parse(value);case"function":return value();default:return value;}
case"text/csv":return d3.csv.parse(value);case"text/tsv":return d3.tsv.parse(value);case"application/xml":return d3.xml.parse(value);}}
ip4.data.TableAdapter=function(){if(ip4.log.log())console.log(this.toString()+" TableAdapter constructor called");}
...
ip4.data.TableAdapter.prototype.x=function(x,parser){if(x===undefined)return this._x;if(ip4.log.log())console.log(this.toString()+" Setting x to: "+[x,parser]);this._x=[x,parser];return this;}
ip4.data.TableAdapter.prototype.y=function(y,parser){if(y===undefined)return this._y;if(ip4.log.log())console.log(this.toString()+" Setting y to: "+[y,parser]);this._y=[y,parser];return this;}
ip4.data.TableAdapter.prototype.group=function(group,parser){if(group===undefined)return this._group;if(ip4.log.log())console.log(this.toString()+" Setting group to: "+[group,parser]);this._group=[group,parser];return this;}
ip4.data.TableAdapter.prototype.adapt=function(data){if(ip4.log.log())console.log(this.toString()+" Adapting data x:",this.x(),", y:",this.y(),", group:",this.group());if(ip4.log.time())console.time(this.toString()+" Adapt");var x=this.x(),y=this.y(),group=this.group();if(x&&y){if(ip4.log.log())console.log(this.toString()+" Adapting x and y...");data.forEach(function(d){d.x=x[1]?x[1](d[x[0]]):d[x[0]];delete d[x[0]];d.y=y[1]?y[1](d[y[0]]):+d[y[0]];delete d[y[0]];});}
if(group&&y){if(ip4.log.log())console.log(this.toString()+" Adapting group and y...");data.forEach(function(d,i){d.group=group[1]?group[1](d[group[0]].trim()):d[group[0]].trim();delete d[group[0]];d.values=y[0].map(function(sub,i){var value={"x":sub,"y":y[1]?y[1](d[sub]):+d[sub]};delete d[sub];return value;});});}
if(group&&x){if(ip4.log.log())console.log(this.toString()+" Adapting group and x...");data=group[0].map(function(serie){return{"group":serie,"values":data.map(function(d){return{"x":x[1]?x[1](d[x[0]].trim()):d[x[0]].trim(),"y":group[1]?group[1](d[serie]):+d[serie]}})}});}
if(ip4.log.time())console.timeEnd(this.toString()+" Adapt");if(ip4.log.log())console.log(this.toString()+" Adapted data is: ",data);return data;}
ip4.data.tableAdapter=function(){return new ip4.data.TableAdapter();}
if(!ip4.decorator)ip4.decorator={}
ip4.decorator.NoSvgSupportDecorator=function(){if(ip4.log.log())console.log(this.toString()+" NoSvgSupportDecorator constructor called");}
ip4.decorator.NoSvgSupportDecorator.prototype={constructor:{configurable:true,enumerable:true,value:ip4.decorator.NoSvgSupportDecorator,writable:true}}
ip4.decorator.NoSvgSupportDecorator.prototype.toString=function(){return"[ip4 NoSvgSupportDecorator]";}
ip4.decorator.NoSvgSupportDecorator.prototype.draw=function(parentElement,configuration){if(ip4.log.log())console.log(this.toString()+" Drawing in parent element",parentElement);var pe=parentElement,con=configuration,noSvgText=(con.alt&&con.alt.noSvgText)?con.alt.noSvgText:"Your browser doesn't support SVG. Use a newer browser to view the chart.";if(con.alt&&con.alt.img){pe.append("figure").append("img").attr({"src":con.alt.img,"title":con.alt.title});}else{pe.append("p").text(noSvgText);}}
ip4.decorator.SvgDecorator=function(){if(ip4.log.log())console.log(this.toString()+" SvgDecorator constructor called");}
ip4.decorator.SvgDecorator.prototype={constructor:{configurable:true,enumerable:true,value:ip4.decorator.SvgDecorator,writable:true}}
ip4.decorator.SvgDecorator.prototype.toString=function(){return"[ip4 SvgDecorator]";}
ip4.decorator.SvgDecorator.prototype.draw=function(parentElement,configuration){if(ip4.log.log())console.log(this.toString()+" Drawing in parentElement:",parentElement,", configuration:",configuration);var css=configuration.style?configuration.style.css:undefined,metadata=configuration.svg?configuration.svg.metadata:undefined;if(css){if(ip4.log.log())console.log(this.toString()+" Adding CSS style.");parentElement.append("style").attr("type","text/css").text(css);}
if(ip4.log.log())console.log(this.toString()+" Adding SVG element.");var svg=parentElement.append("svg");this.g=svg.append("g").attr("class","main");configuration.svg.size.width=configuration.svg.size.width||parentElement.node().offsetWidth;configuration.svg.size.height=configuration.svg.size.height||configuration.svg.size.width*0.6;if(ip4.log.log())console.log(this.toString()+" Setting svg width,height to: "+configuration.svg.size.width+","+configuration.svg.size.height);svg.attr("width",configuration.svg.size.width);svg.attr("height",configuration.svg.size.height);if(configuration.svg.size.margin){this.g.attr("transform","translate("+(configuration.svg.size.margin.left||0)
+","+(configuration.svg.size.margin.top||0)+")");}
if(metadata){if(ip4.log.log())console.log(this.toString()+" Adding SVG metadata.");d3.ns.prefix.rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#";d3.ns.prefix.rdfs="http://www.w3.org/2000/01/rdf-schema#";d3.ns.prefix.dc="http://purl.org/dc/elements/1.1/";var description=svg.append("metadata").append("rdf:RDF").attr("xmlns:rdf",d3.ns.prefix.rdf.space).attr("xmlns:rdfs",d3.ns.prefix.rdfs.space).attr("xmlns:dc",d3.ns.prefix.dc.space).append("rdf:Description");for(var prop in metadata){description.attr(prop,metadata[prop]);}}}
ip4.decorator.AxisDecorator=function(){if(ip4.log.log())console.log(this.toString()+" AxisDecorator constructor called");}
...
ip4.decorator.TooltipDecorator=function(){if(ip4.log.log())console.log(this.toString()+" TooltipDecorator constructor called");}
...
ip4.decorator.TooltipDecorator.prototype.draw=function(){if(ip4.log.log())console.log(this.toString()+" Drawing...");d3.select("body").append("div").attr("class","ip4 tooltip").on("mousedown",function(d){d3.select(this).classed("selected",false);d3.event.sourceEvent.stopPropagation();});}
if(!ip4.template)ip4.template={};ip4.template.BarChart=function(){if(ip4.log.log())console.log(this.toString()+" Constructor called");ip4.document.Document.call(this);this.svgDecorator=new ip4.decorator.SvgDecorator();this.axisDecorator=new ip4.decorator.AxisDecorator();this.tooltipDecorator=new ip4.decorator.TooltipDecorator();}
ip4.template.BarChart.prototype=Object.create(ip4.document.Document.prototype,{constructor:{configurable:true,enumerable:true,value:ip4.template.BarChart,writable:true}});ip4.template.BarChart.prototype.toString=function(){return"[ip4 BarChart]";}
ip4.template.BarChart.prototype.defaultConfiguration=function(){this.parent.classed("barChart",true);return{"data":{"value":function(){return[{"x":"Group A","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group B","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group C","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group D","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group E","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group F","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group G","y":ip4.util.getRandomMinMax(-1000,1000)},{"x":"Group H","y":ip4.util.getRandomMinMax(-1000,1000)}]}},"svg":{"size":{"margin":{"top":20,"right":10,"bottom":40,"left":60}},"metadata":{"dc:format":"application/ip4"}},"d3":{"xScale":d3.scale.ordinal(),"innerBands":0.15,"outerBands":0.3,"yScale":d3.scale.linear().nice(),"yLabel":"","xAxis":d3.svg.axis(),"yAxis":d3.svg.axis().orient("left").tickFormat(d3.format(",.2f")),"color":d3.scale.ordinal().range(ip4.color.random(20)),"transitions":{"groups":d3.transition().duration(1500).ease("linear"),"xAxis":d3.transition().duration(1500).ease("linear"),"yAxis":d3.transition().duration(1500).ease("linear"),"zeroline":d3.transition().duration(1500).ease("linear")}}}}
ip4.template.BarChart.prototype.draw=function(){if(ip4.log.log())console.log(this.toString()+" Drawing with parentElement:",this.parentElement(),", configuration:",this.configuration());if(ip4.log.time())console.time(this.toString()+" Draw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,x=con.d3.xScale,y=con.d3.yScale,innerBands=con.d3.innerBands,outerBands=con.d3.outerBands,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,yLabel=con.d3.yLabel,updateInterval=con.data.updateInterval;if(!ip4.util.isSvgSupport()){if(ip4.log.warn())console.warn(this.toString()+" Browser has no SVG support");new ip4.decorator.NoSvgSupportDecorator().draw(parent,con);return;}
this.svgDecorator.draw(parent,con);var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom;x.rangeRoundBands([0,w],innerBands,outerBands);y.range([h,0]);this.axisDecorator.draw(parent.select("g.main"),xAxis,yAxis,x,y,w,h,null,yLabel);this.tooltipDecorator.draw();if(updateInterval&&updateInterval>0.0){if(ip4.log.info())console.info(this.toString()+" Starting data update interval "+updateInterval+" ms...");var self=this;window.setInterval(function(){self.update();},+updateInterval);}
if(ip4.log.info())console.info(this.toString()+" Updating...");this.update();if(ip4.log.time())console.timeEnd(this.toString()+" Draw");return this;}
ip4.template.BarChart.prototype.redraw=function(data){if(ip4.log.log())console.log(this.toString()+" Redrawing...");if(ip4.log.time())console.time(this.toString()+" Redraw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,x=con.d3.xScale,y=con.d3.yScale,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,color=con.d3.color,transitions=con.d3.transitions,updateInterval=con.data.updateInterval;var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom;var g=parent.select("g.main"),tooltip=d3.selectAll(".tooltip",".ip4");x.domain(data.map(function(d){return d.x;}));y.domain([d3.min(data,function(d){return d.y>0?0:d.y;}),d3.max(data,function(d){return d.y;})]);var groups=g.selectAll(".group").data(data,function(d){return d.x;});groups.attr("class","group update").attr("width",x.rangeBand()).attr("x",function(d){return x(d.x);});groups.enter().append("rect").attr("class","group enter").attr("width",x.rangeBand()).attr("x",function(d){return x(d.x);}).style("fill",function(d){return color(d.x);}).on("mouseover",function(d){d3.select(this).classed("selected",true);tooltip.html((xAxis.tickFormat()?xAxis.tickFormat()(d.x):d.x)
+"<br/>"
+(yAxis.tickFormat()?yAxis.tickFormat()(d.y):d.y)).style("left",(d3.event.pageX+7)+"px").style("top",(d3.event.pageY-28)+"px").classed("selected",true);}).on("mousemove",function(d){tooltip.style("left",(d3.event.pageX+7)+"px").style("top",(d3.event.pageY-28)+"px");}).on("mouseout",function(d){d3.select(this).classed("selected",false);tooltip.html("").classed("selected",false);});groups.exit().attr("class","group exit").remove();transitions.groups.each(function(){groups.transition().attr("y",function(d){return d.y>0?y(d.y):y(0);}).attr("height",function(d){return d.y>0?h-y(d.y+y.domain()[0]):h-y(y.domain()[0]-d.y);})});this.axisDecorator.redraw(data,g,xAxis,yAxis,x,y,w,h,transitions);if(ip4.log.time())console.timeEnd(this.toString()+" Redraw");return this;}
ip4.template.barChart=function(){return new ip4.template.BarChart();}
ip4.barChart=function(){return ip4.template.barChart();}
if(!ip4.template)ip4.template={};ip4.template.LineChart=function(){if(ip4.log.log())console.log(this.toString()+" Constructor called");ip4.document.Document.call(this);this.svgDecorator=new ip4.decorator.SvgDecorator();this.axisDecorator=new ip4.decorator.AxisDecorator();this.tooltipDecorator=new ip4.decorator.TooltipDecorator();}
ip4.template.LineChart.prototype=Object.create(ip4.document.Document.prototype,{constructor:{configurable:true,enumerable:true,value:ip4.template.LineChart,writable:true}});ip4.template.LineChart.prototype.toString=function(){return"[ip4 LineChart]";}
ip4.template.LineChart.prototype.defaultConfiguration=function(){this.parent.classed("lineChart",true);var min=-1000,max=1000;return{"data":{"value":function(){return[{"x":ip4.util.getRandomMinMax(-0.2,0.2),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(0.3,0.7),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(0.8,1.2),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(1.3,1.7),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(1.8,2.2),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(2.3,2.7),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(2.8,3.2),"y":ip4.util.getRandomMinMax(min,max)},{"x":ip4.util.getRandomMinMax(3.3,3.7),"y":ip4.util.getRandomMinMax(min,max)}]}},"svg":{"size":{"margin":{"top":20,"right":10,"bottom":40,"left":60}},"metadata":{"dc:format":"application/ip4"}},"d3":{"xScale":d3.scale.linear().nice(),"yScale":d3.scale.linear().nice(),"line":d3.svg.line().interpolate("linear"),"color":ip4.color.random(),"dotRadius":6,"xLabel":"","yLabel":"","xAxis":d3.svg.axis().tickFormat(d3.format(",.2f")),"yAxis":d3.svg.axis().orient("left").tickFormat(d3.format(",.2f")),"transitions":{"lines":d3.transition().duration(1500).ease("linear"),"dots":d3.transition().duration(1500).ease("linear"),"xAxis":d3.transition().duration(1500).ease("linear"),"yAxis":d3.transition().duration(1500).ease("linear"),"zeroline":d3.transition().duration(1500).ease("linear")}}}}
ip4.template.LineChart.prototype.draw=function(){if(ip4.log.log())console.log(this.toString()+" Drawing with parentElement:",this.parentElement(),", configuration:",this.configuration());if(ip4.log.time())console.time(this.toString()+" Draw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,x=con.d3.xScale,y=con.d3.yScale,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,xLabel=con.d3.xLabel,yLabel=con.d3.yLabel,updateInterval=con.data.updateInterval;if(!ip4.util.isSvgSupport()){if(ip4.log.warn())console.warn(this.toString()+" Browser has no SVG support");new ip4.decorator.NoSvgSupportDecorator().draw(parent,con);return;}
this.svgDecorator.draw(parent,con);var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom;x.range([0,w]);y.range([h,0]);this.axisDecorator.draw(parent.select("g.main"),xAxis,yAxis,x,y,w,h,xLabel,yLabel);this.tooltipDecorator.draw();parent.select("svg").append("defs").append("clipPath").attr("id","clip").append("rect").attr("width",w+10).attr("height",h+10).attr("transform","translate(0,-10)");if(updateInterval&&updateInterval>0.0){if(ip4.log.info())console.info(this.toString()+" Starting data update interval "+updateInterval+" ms...");var self=this;window.setInterval(function(){self.update();},+updateInterval);}
if(ip4.log.info())console.info(this.toString()+" Updating...");this.update();if(ip4.log.time())console.timeEnd(this.toString()+" Draw");return this;}
ip4.template.LineChart.prototype.redraw=function(data){if(ip4.log.log())console.log(this.toString()+" Redrawing...");if(ip4.log.time())console.time(this.toString()+" Redraw");var parent=this.parentElement(),con=this.configuration(),size=con.svg.size,x=con.d3.xScale,y=con.d3.yScale,line=con.d3.line,color=con.d3.color,dotRadius=con.d3.dotRadius,xAxis=con.d3.xAxis,yAxis=con.d3.yAxis,xLabel=con.d3.xLabel,yLabel=con.d3.yLabel,transitions=con.d3.transitions,updateInterval=con.data.updateInterval;var w=size.width-size.margin.right-size.margin.left,h=size.height-size.margin.top-size.margin.bottom;var g=parent.select("g.main"),tooltip=d3.selectAll(".tooltip",".ip4");line.x(function(d){return x(d.x);}).y(function(d){return y(d.y);});x.domain(d3.extent(data,function(d){return d.x;}));y.domain(d3.extent(data,function(d){return d.y;}));this.axisDecorator.redraw(data,g,xAxis,yAxis,x,y,w,h,transitions);var lines=g.selectAll(".line").data(data);var dots=g.selectAll(".dot").data(data);lines.attr("class","line update").style("stroke",function(d){return color;});dots.attr("class","dot update");var path=lines.enter().append("g").attr("clip-path","url(#clip)").append("path").attr("class","line enter").style("stroke",function(d){return color;}).attr("d",line(data));dots.enter().append("g").attr("clip-path","url(#clip)").append("circle").attr("class","dot enter").style("fill",function(d){return color;}).attr("cx",function(d){return x(d.x);}).attr("cy",function(d){return y(d.y);}).attr("r",dotRadius).on("mouseover",function(d){d3.select(this).classed("selected",true);tooltip.html(xLabel.substring(0,20)+": "+(xAxis.tickFormat()?xAxis.tickFormat()(d.x):d.x)
+"<br/>"+yLabel.substring(0,40)+": "
...
ip4.template.lineChart=function(){return new ip4.template.LineChart();}
ip4.lineChart=function(){return ip4.template.lineChart();}